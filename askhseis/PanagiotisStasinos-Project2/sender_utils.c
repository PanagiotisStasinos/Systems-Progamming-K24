...
int create_file_list(char* dir, struct list_head* head){	//anadromikh, otan vriskei subdir
	char *path;
	...
	char *file = NULL;
	while ((ent = readdir (input_dir_ptr)) != NULL) {	//gia kathe kainourgio id kai subdir sto common dir
...
int insert_file(char* file_path, int size, struct list_head* head){
	struct list_node* nd = head->first;
	char *c;
	c = file_path;
	...
	nd->next = (struct list_node*)malloc(sizeof(struct list_node));
	nd->next->size = size;
